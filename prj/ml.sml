...
| eval(letFun(var,body,ENV)) =
    ...
    in (*extracting variable parameters from var *)
        let val vars =
...
| eval (apply((Sym h),t)) = 
    let fun getFun(h,t): lisp = case h of
...
| eval (applyFun(x,args,ENV)) =
    ...
    let fun f(x,args , ENV) = case x of
        ...
        if (len var) = (len args) then (*fill the enviroment *)
            let val Env = 
            ...
            in 
                let fun getEnv ((Var v), env) = case env of
                        none => raise Fail "Empty environment or variable not found"
                        | cons (h,t) => 
                        if  v = let fun getVar(Var x) = x in getVar(eval(car(h))) end then
                            eval (car (cdr h) )
                        else getEnv( (Var v), t )
                    ...
                    let fun evalExp(body,res) = case body of
                        ...
                        if isVar(x) then
                            let val X = getEnv(x,Env) in
                                if isVar(y) then
                                    let val Y = getEnv(y,Env) in eval(applyFun(X,cons(Y,none),Env)) end
                        ...
                        if isVar(var) then
                            let val VAR = getEnv(var,Env) in
                                if isVar(body) then
                                    let val Body=getEnv(body,Env) in eval(letFun(VAR,Body,Env)) end
...
| pretty (quote l) = let fun printQuote (Unit u) = "()"
        ...
        | printQuote (cons(h,t)) = 
            let fun printList(lst:lisp):string = case lst of
                none => "none)"
...
| pretty (cons(h,t)) = "("^ 
    let fun printCons(lst:lisp):string = case lst of
        none => ""
